...
import { Pencil, Trash2, Plus, Image as ImageIcon, FileText, Info, X, ZoomIn } from "lucide-react";
...
function saveNoteGroups(groups: string[]) { localStorage.setItem(NOTE_GROUPS_KEY, JSON.stringify(groups)); }
  ...
  return (
    <Dialog open onOpenChange={o => !o && onClose()}>
      <DialogContent className="max-w-4xl p-2 bg-black/90 border-0">
        <img src={src} alt="Full size" className="w-full h-auto object-contain max-h-[85vh] rounded-lg" />
      </DialogContent>
    </Dialog>
  );
}
...
export default function Notes() {
  ...
  const handleAddCustomGroup = () => {
    ...
    setNoteGroups(updated); saveNoteGroups(updated);
  ...
  const handleAddNote = async () => {
    ...
    setAddTarget(null); setNewClassLabel(""); setNewContent(""); setNewPhotoUrl(null);
    queryClient.invalidateQueries({ queryKey: getListNotesQueryKey() });
  };

  const getNotesForSubject = (id: number) => allNotes?.filter(n => n.subjectId === id) || [];
  const getNotesForGroup = (name: string) => allNotes?.filter(n => n.subjectId === null && n.noteGroupName === name) || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl bg-primary/5 border border-primary/20 text-sm text-muted-foreground">
        ...
        <span>Subjects from <strong className="text-foreground">Course Outline</strong> appear here automatically. Tap a photo to view it full-size.</span>
      </div>

      {addingGroup && (
        <div className="flex gap-2 p-3 bg-muted/30 border border-border rounded-xl">
          <Input autoFocus placeholder="Group name" value={newGroupName} onChange={e => setNewGroupName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAddCustomGroup()} />
      ...
      {!hasAnything ? (
        ...
        <div className="grid gap-6">
          {subjects?.map(subject => {
            ...
            return (
              <NoteGroup
                key={`s-${subject.id}`} title={subject.name} notes={notes} isAdding={isAdding}
                newClassLabel={newClassLabel} newContent={newContent} newPhotoUrl={newPhotoUrl} fileRef={fileRef}
                ...
                onSetClassLabel={setNewClassLabel} onSetContent={setNewContent} onPhotoUpload={handlePhotoUpload}
                onSave={handleAddNote} onCancel={() => setAddTarget(null)} isSaving={createNote.isPending}
          ...
          {noteGroups.map(groupName => {
            ...
            return (
              <NoteGroup
                key={`g-${groupName}`} title={groupName} notes={notes} isCustom
                onRemoveGroup={() => { const u = noteGroups.filter(g => g !== groupName); setNoteGroups(u); saveNoteGroups(u); }}
                isAdding={isAdding} newClassLabel={newClassLabel} newContent={newContent} newPhotoUrl={newPhotoUrl} fileRef={fileRef}
...
function NoteGroup({ title, notes, isCustom, onRemoveGroup, isAdding, newClassLabel, newContent, newPhotoUrl, fileRef, onStartAdd, onSetClassLabel, onSetContent, onPhotoUpload, onSave, onCancel, isSaving, onOpenPhoto }: any) {
  return (
      ...
      <CardContent className="pt-5">
        {notes.length > 0 ? (
          <div className="grid gap-4 sm:grid-cols-2 mb-5">
            {notes.map((note: any) => <NoteCard key={note.id} note={note} onOpenPhoto={onOpenPhoto} />)}
          </div>
        ) : (
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => fileRef.current?.click()}>
                  <ImageIcon className="w-4 h-4 mr-2" />{newPhotoUrl ? "Change Photo" : "Add Photo"}
                </Button>
...
function NoteCard({ note, onOpenPhoto }: { note: any; onOpenPhoto: (src: string) => void }) {
  ...
  const handleUpdate = () => {
    updateNote.mutate(
      ...
      { onSuccess: () => { setIsEditing(false); queryClient.invalidateQueries({ queryKey: getListNotesQueryKey() }); } }
    );
  };
  const handleDelete = () => {
    deleteNote.mutate({ id: note.id }, {
      onSuccess: () => { setShowDelete(false); queryClient.invalidateQueries({ queryKey: getListNotesQueryKey() }); },
    });
  };
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <>
      <div className="bg-card p-4 rounded-xl border border-border shadow-sm group flex flex-col">
        ...
        {note.content && <p className="text-sm text-muted-foreground whitespace-pre-wrap mb-3 flex-1">{note.content}</p>}
        {photoSrc && (
          <button
            className="mt-2 rounded-lg overflow-hidden border border-border/50 bg-muted/20 relative group/photo cursor-zoom-in w-full"
            onClick={() => onOpenPhoto(photoSrc)}
          >
            <img src={photoSrc} alt="Note attachment" className="w-full object-contain max-h-52" />
            <div className="absolute inset-0 bg-black/0 group-hover/photo:bg-black/20 transition-colors flex items-center justify-center">
              <ZoomIn className="w-6 h-6 text-white opacity-0 group-hover/photo:opacity-100 transition-opacity drop-shadow-lg" />
            </div>
          </button>
        )}
      ...
      <AlertDialog open={showDelete} onOpenChange={setShowDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>"{note.classLabel}" and any attached photo will be permanently deleted.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
